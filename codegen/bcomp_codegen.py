...
##############################################################################
# Project:  bytesfunc
# Purpose:  Generate the C code for eq, ge, gt, le, lt, ne.
# Language: Python 3.6
# Date:     02-Nov-2019
#
###############################################################################
...
char %(funclabel)s_1(Py_ssize_t arraylen, unsigned char *data1, unsigned char param) {
	...
	for(x = 0; x < arraylen; x++) {
		if (!(data1[x] %(compare_ops)s param)) { return 0; }
	}
...
char %(funclabel)s_3(Py_ssize_t arraylen, unsigned char param, unsigned char *data2) {
	...
	for(x = 0; x < arraylen; x++) {
		if (!(param %(compare_ops)s data2[x])) { return 0; }
	}
...
char %(funclabel)s_5(Py_ssize_t arraylen, unsigned char *data1, unsigned char *data2) {
	...
	for(x = 0; x < arraylen; x++) {
		if (!(data1[x] %(compare_ops)s data2[x])) { return 0; }
	}
...
static PyObject *py_%(funclabel)s(PyObject *self, PyObject *args, PyObject *keywds) {
	...
	bytesdata = getparams_comp(self, args, keywds, "%(funclabel)s");
...
\\n\\
======================  ============================================== \\n\\
Equivalent to:          all([x %(compare_ops)s param for x in sequence]) \\n\\
or                      all([param %(compare_ops)s x for x in sequence]) \\n\\
or                      all([x %(compare_ops)s y for x,y in zip(sequence1, sequence2)]) \\n\\
======================  ============================================== \\n\\
\\n\\
Call formats: \\n\\
\\n\\
  result = %(funclabel)s(sequence1, param) \\n\\
  result = %(funclabel)s(param, sequence1) \\n\\
  result = %(funclabel)s(sequence1, sequence2) \\n\\
  result = %(funclabel)s(sequence1, param, maxlen=y) \\n\\
  result = %(funclabel)s(sequence1, param, nosimd=False) \\n\\
\\n\\
...
'ge' : '0xffffffffffffffff',
'gt' : '0xffffffffffffffff',
'le' : '0xffffffffffffffff',
'lt' : '0xffffffffffffffff',
'ne' : '0x0000000000000000',
}

# ==============================================================================

for funcname in compare_ops.keys():

	filename = funcname + '.c'

	with open(filename, 'w') as f:

		opvals = {'funclabel' : funcname, 
				'compare_ops' : compare_ops[funcname],
				'SIMD_x86_arr_num' : SIMD_x86_arr_num[funcname],
